...
// KEY FIX: Added norm_size parameter for variance normalization
...
// KEY FIX: Added sqrt(softmax_weight) and norm_size parameter
__global__ void accumulate_perturbation_2d(
    ...
    for (i32 b = 0; b < batch_size; b++) {
        ...
        perturb_sum += sqrtf(softmax_weights[b]) * p;  // KEY FIX: sqrt
    }
    ...
    for (i32 out_chunk = 0; out_chunk < num_out_chunks; out_chunk++) {
        ...
        for (i32 in_chunk = 0; in_chunk < 12; in_chunk++) {
            ...
            for (i32 i = tid; i < 256; i += 32) {
                ...
                f32 perturb = get_perturbation(base_seed, batch_idx, layer_id, weight_row, weight_col, perturb_scale, norm_size);
...
__global__ void final_projection(
    ...
    for (i32 out_chunk = 0; out_chunk < 4; out_chunk++) {
        ...
        for (i32 i = tid; i < 256; i += 32) {
            ...
            f32 perturb = get_perturbation(base_seed, batch_idx, layer_id, row_base + row, col, perturb_scale, norm_size);
            weight_smem[i] = quantize(base_w + perturb, inv_scale);
        }
        __syncthreads();
...
__global__ void materialize_image(
    ...
    for (i32 pixel_idx = tid; pixel_idx < 192; pixel_idx += 32) {
        ...
        for (i32 p = 0; p < 64; p++) {
            ...
            patch_pixel += get_perturbation(base_seed, batch_idx, layer_id, p, pixel_idx, perturb_scale, norm_size);
...
__global__ void compute_qkv_batched(
    ...
    for (i32 in_head = 0; in_head < num_heads; in_head++) {
        ...
        for (i32 i = tid; i < 256; i += 32) {
            ...
            f32 perturb = get_perturbation(base_seed, batch_idx, layer_id, q_row_base + row, col, perturb_scale, norm_size);
            weight_smem[i] = quantize(base_w + perturb, inv_scale);
        }
        __syncthreads();
        ...
        for (i32 i = tid; i < 256; i += 32) {
            ...
            f32 perturb = get_perturbation(base_seed, batch_idx, layer_id, k_row_base + row, col, perturb_scale, norm_size);
            weight_smem[i] = quantize(base_w + perturb, inv_scale);
        }
        __syncthreads();
        ...
        for (i32 i = tid; i < 256; i += 32) {
            ...
            f32 perturb = get_perturbation(base_seed, batch_idx, layer_id, v_row_base + row, col, perturb_scale, norm_size);
            weight_smem[i] = quantize(base_w + perturb, inv_scale);
        }
        __syncthreads();
...
// KEY FIX: Added ReLU before projection
__global__ void project_and_residual(
    ...
    for (i32 head_idx = 0; head_idx < num_heads; head_idx++) {
        ...
        for (i32 i = tid; i < 256; i += 32) {
            // KEY FIX: Apply ReLU to attention output
            f32 attn_val = static_cast<f32>(attn_output[attn_offset + i]) * scale;
            attn_smem[i] = quantize(fmaxf(0.0f, attn_val), inv_scale);
            
            const i32 row = i / 16;
            const i32 col = i % 16;
            f32 base_w = static_cast<f32>(proj_weights[proj_offset + i]) * scale;
            f32 perturb = get_perturbation(base_seed, batch_idx, layer_id, proj_row_base + row, col, perturb_scale, norm_size);
...
cpp_source = """
//js
torch::Tensor patchify_fn(int64_t stream_ptr, torch::Tensor images, torch::Tensor proj_weights, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, double pe_scale, int64_t norm_size);
torch::Tensor final_projection_fn(int64_t stream_ptr, torch::Tensor tokens, torch::Tensor proj_weights, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
torch::Tensor materialize_image_fn(int64_t stream_ptr, torch::Tensor patch_weights, torch::Tensor learnable_patches, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
std::vector<torch::Tensor> compute_qkv_batched_fn(int64_t stream_ptr, torch::Tensor tokens, torch::Tensor QKV_weights, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
torch::Tensor flash_attention_batched_fn(int64_t stream_ptr, torch::Tensor Q, torch::Tensor K, torch::Tensor V, double max_val);
torch::Tensor project_and_residual_fn(int64_t stream_ptr, torch::Tensor attn_output, torch::Tensor proj_weights, torch::Tensor residual, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
...
torch::Tensor accumulate_perturbation_2d_fn(int64_t stream_ptr, torch::Tensor base_weights, torch::Tensor softmax_weights, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
torch::Tensor accumulate_perturbation_3d_fn(int64_t stream_ptr, torch::Tensor base_weights, torch::Tensor softmax_weights, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
torch::Tensor accumulate_perturbation_5d_fn(int64_t stream_ptr, torch::Tensor base_weights, torch::Tensor softmax_weights, double max_val, double perturb_scale, int64_t seed, int64_t layer_id, int64_t norm_size);
;//
"""

module = load_inline(
    name="vit_int8_evolutionary_v11",
...
class ViTWeights:
    def __init__(self, num_heads, num_layers, max_val, device="cuda"):
        ...
        self.device = device
        self.hidden_dim = num_heads * 16
        
        # Norm sizes for each layer type
        self.norm_sizes = {
            'patchifier': 192,
            'final_proj': 16,
            'patches': 8,
            'qkv': self.hidden_dim,
            'proj': self.hidden_dim,
        }
    ...
    def accumulate_perturbations(self, softmax_weights, seed, perturb_scale, stream):
        ...
        self.patchifier = module.accumulate_perturbation_2d_fn(
            ...
            self.max_val, perturb_scale, seed, 0, self.norm_sizes['patchifier']
        )
        
        self.final_proj = module.accumulate_perturbation_3d_fn(
            stream, self.final_proj, softmax_weights,
            self.max_val, perturb_scale, seed, 1, self.norm_sizes['final_proj']
        )
        ...
        patches_updated = module.accumulate_perturbation_2d_fn(
            ...
            self.max_val, perturb_scale, seed, 2, self.norm_sizes['patches']
        )
        ...
        for i in range(self.num_layers):
            ...
            self.qkv_weights[i] = module.accumulate_perturbation_5d_fn(
                ...
                self.max_val, perturb_scale, seed, layer_base, self.norm_sizes['qkv']
            )
            
            self.proj_weights[i] = module.accumulate_perturbation_3d_fn(
                stream, self.proj_weights[i], softmax_weights,
                self.max_val, perturb_scale, seed, layer_base + 1, self.norm_sizes['proj']
            )
...
def forward_pass(images, weights, seed, perturb_scale, pe_scale, stream):
    ...
    tokens = module.patchify_fn(
        ...
        max_val, perturb_scale, seed, 0, pe_scale, weights.norm_sizes['patchifier']
    )
...
# ============ Test ============
print("=" * 60)
print("Evolutionary Int8 ViT Training v11")
print("Key fixes: normalization, sqrt(softmax), ReLU, tanh")
print("=" * 60)
...
for epoch in range(num_epochs):
    ...
    # Input with noise
    noise_sigma = 0.01
    images = ground_truth.float() / 127.0
    ...
    if epoch % 100 == 0 or epoch == num_epochs - 1:
        try:
            dds = dds_from_tensor(output[0:1, :, :, :].permute(0, 3, 1, 2).float() / 127.0)
        ...
        print(f"  Best MSE: {best_mse:.4f}, Mean MSE: {mean_mse:.4f}")
        ...
        print(f"  Top 5 weights: {np.sort(softmax_np)[-5:][::-1].round(4)}")
