...
class Model(nn.Module):
    ...
    def forward(self, input):
        ...
        # Latent space
        latents_mu     = self.fc_mu(x)
        latents_logvar = self.fc_logvar(x)

        # Reparameterization trick
        if self.training:
            std = torch.exp(0.5 * latents_logvar)
            eps = torch.randn_like(std)
            x   = latents_mu + eps * std
        else:
            x = latents_mu
        ...
        return torch.lerp(input, y, alpha), latents_mu, latents_logvar
...
for epoch in range(num_epochs):
    ...
    if 0:
        ...
        optimizer.zero_grad()
        o, latents_mu, latents_logvar           = model(b)
    ...
    for iter in range(iters // chain):
        ...
        for chidx in range(chain):
            _i = o
            o, latents_mu, latents_logvar           = model(o)
            schedule_weight = (math.exp(4.0 * (iter * chain + chidx - iters ) / (iters))) * 16.0
            loss            = loss + (F.mse_loss(o, b)) * schedule_weight
            ...
            # loss  = loss + torch.mean((o - _i).square()) * 0.1

            if 0:
                # Contrastive flow matching loss
                # Encourage diversity in the batch by pushing features apart
                cfm_target = torch.roll(o, shifts=1, dims=0)
                cfm_loss = -((o - cfm_target).square()).mean() * 0.1
                loss     = loss + cfm_loss
